...
class MenuItemController extends Controller
{
  ...
  {
    $alert = count($this->menu->findAllWhere('created_by', Auth::id())) === 0;

    return view('menu_items/create', [
      'alert' => $alert,
      'posts' =>
        $this->posts->findAllWhereAnd(
          'user_id',
          Auth::id(),
          'post_status',
          'published'
  ...
  private function getDataForm(): array
  {
    return [
      ...
      'link' => $this->post('link'),
      'target_window' =>
        $this->post('target_window') !== null
          ? $this->post('target_window')
          : 0,
      'menu_id' => $this->post('menu_id'),
      'parent_item' => $this->post('parent_item') !== ''
        ? $this->post('parent_item')
        : null,
      'default_page' => $this->post('default_page') !== null
        ? $this->post('default_page')
        : 0,
